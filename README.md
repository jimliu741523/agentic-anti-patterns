...
- AP-44 — does the orchestrator track per-agent task-type performance at runtime, or does it assign subtasks by round-robin / role label alone? Is expertise-weighted voting used for final decisions, or is unweighted majority the default? Are minority expert opinions surfaced before consensus is committed on high-stakes subtasks?
- AP-45 — when a multi-agent pipeline has sequential phases, is there an explicit `Barrier(n_agents, phase_id)` that blocks phase N+1 from starting until all phase-N agents have committed? Do phase-N agents write an output manifest before calling barrier.arrive? Is a watchdog recovering from crashed agents (stale leases) rather than silently skipping their outputs?
- AP-46 — does the work queue attach a TTL to every claim, or does an uncompleted claim hold permanently if the claiming agent dies? Is there a heartbeat/lease-renewal protocol so a dead agent's claim is distinguished from an active one? Is there a watchdog that detects stale claims and reassigns the task rather than letting the orchestrator block indefinitely? When max_attempts is exhausted, does the pipeline emit DEGRADED rather than silently freezing?
- AP-47 — before parallel agents are dispatched, is there a shared task contract (structured scope, output format, constraint priority) that each agent must acknowledge? Is a semantic compatibility check (pairwise cosine similarity ≥ 0.65) run across agent outputs before the combiner step? Are combiner errors that persist across 3+ retries flagged as semantic failures rather than transient ones? Is mean pairwise output similarity tracked over time as a pipeline health metric?
...
The original 14-entry roadmap plus AP-15..AP-47 are shipped. Future entries are demand-driven (PRs welcome) — open an issue with a candidate failure mode + a real incident or reproduction.
