...
- AP-48 — is there a pre-execution gate that checks the model's selection-confidence before committing to tool execution? Is the activation gap (or logit entropy on the tool-name tokens) logged per call so low-confidence selections can be flagged for clarification? When the gate is unavailable, is a Reason-then-Act wrapper injected before tool-name generation to surface the model's internal necessity signal? Is the wrong-tool rate stratified by selection-confidence decile and monitored for systematic low-confidence exposure in a specific tool class?
...
**Symptom.** A second-phase agent (aggregator, reviewer, compiler) begins processing while some first-phase agents are still writing. Its output reflects a partial set of results — missing contributions from slower or crashed agents. The pipeline produces a "complete" result that silently omits content from late-completing agents. No exception, retry, or timeout fires; the pipeline exits with an exit code of 0.

**Example.** A multi-agent code-review pipeline: Phase 1 — four specialist agents (security, performance, style, logic) each review a PR and write a review file. Phase 2 — a single aggregation agent reads all four review files and writes a summary. The aggregator starts after the first two files appear in the output directory, before security and logic agents finish writing. The aggregated summary omits the security findings entirely. The PR is approved with an incomplete review. No log records that the aggregator ran before all phase-N inputs were ready.

**Root cause.**
- Orchestration frameworks do not expose a barrier primitive — no `Barrier(n_agents, phase_id)` exists that blocks phase N+1 execution until all n phase-N agents have committed their outputs.
- Practitioners substitute wall-clock sleep or item-count polling — neither guarantees all agents have finished writing; a crashed agent leaves its slot empty and the count-based check still appears satisfied at N-1.
- File-based coordination is especially fragile: directory listings report partial writes as complete, OS-level buffering delays visibility, and concurrent file creation produces race conditions on the item count.
- When a phase-N agent crashes mid-write, its partially-written output may be visible (file exists, non-zero size) while being semantically incomplete, causing downstream consumption of corrupted data without an error signal.

**Mitigations.**
1. **Explicit barrier primitive.** Implement `Barrier(n_agents, phase_id)` backed by a SQLite `phase_arrivals` table: `(phase_id, agent_id, arrived_at, ttl_seconds)`. Phase-N agents call `barrier.arrive(phase_id)` only after writing and fsync-ing their full output. Phase N+1 is unblocked only when `COUNT(DISTINCT agent_id) = n_agents` for that `phase_id`. No polling, no wall-clock sleep.
2. **Output manifest before barrier arrival.** Each phase-N agent writes a manifest file listing all artifact keys it produced, then calls `barrier.arrive()`. The barrier validator checks that all manifested keys exist on disk before releasing phase N+1. A missing key holds the barrier and fires an alert rather than silently releasing with incomplete data.
3. **Crash-recovery watchdog.** Each barrier arrival includes a TTL-bearing lease. A watchdog detects expired leases (phase-N agents that accepted a task but did not arrive within `ttl_seconds`) and either re-queues the failed agent's work item or marks the pipeline as `DEGRADED` rather than silently advancing with N-1 inputs.
4. **Phase commit tokens for stale-write rejection.** Each phase-N agent receives a monotonically increasing commit token at task assignment. Phase N+1 agents only consume inputs whose commit token matches the current phase epoch. Stale writes from a previous run that flushed late (OS-buffered partial writes) are rejected by token mismatch before being incorporated into phase-N+1 output.

**Detection.**
- **Phase advancement timestamp ordering.** Log `phase_N_last_arrive` and `phase_N1_first_start`. If `phase_N1_first_start < phase_N_last_arrive` for any run, a phase-gate violation occurred — phase N+1 started before all phase-N agents finished.
- **Output completeness audit.** After every pipeline run, verify that every phase-N agent's output manifest appears as a confirmed input read by the phase-N+1 agent. A read/write count mismatch (phase N+1 read K inputs, phase N produced N inputs, K < N) is the phase-gate absence signature.
- **Crash-incomplete rate.** Track what fraction of phase-N agents do not produce a committed output manifest before the next phase starts. Any value above 0% without a corresponding `DEGRADED` pipeline flag indicates the barrier is absent or non-enforcing.
- **Aggregator input count vs. expected.** If the phase-N+1 aggregator consistently receives fewer inputs than `n_agents` — and the pipeline still exits 0 — a barrier absence is allowing premature advancement with partial data.

**Related.**
- [AP-27 — Multi-agent concurrent state corruption](#ap-27--multi-agent-concurrent-state-corruption): AP-27 is concurrent within-phase write conflicts — parallel agents in the same phase overwriting each other's work on a shared artifact. AP-45 is a temporal ordering failure across phases — downstream agents advancing before upstream agents finish, with no shared-artifact conflict.
- [AP-36 — Agent capacity overload cascade (absent backpressure primitives)](#ap-36--agent-capacity-overload-cascade-absent-backpressure-primitives): AP-36 is throughput collapse when a downstream agent is saturated by excessive incoming work. AP-45 is a correctness failure from premature phase advancement — the downstream agent is not overloaded, it simply reads incomplete inputs.
- [AP-31 — Hallucinated multi-agent consensus](#ap-31--hallucinated-multi-agent-consensus): AP-31 is agents verbally reporting agreement without committing state. AP-45 is the phase-N+1 agent proceeding with real but incomplete committed state — no false agreement signal, just an incomplete input set.
- [AP-42 — Multi-agent failure attribution blackout](#ap-42--multi-agent-failure-attribution-blackout): AP-42 is the forensics problem — after a failure, which agent/step was responsible? AP-45 is the prevention problem — ensuring all phase-N work is complete before phase N+1 begins. Output manifests and barrier arrival logs (AP-45 mitigation) directly feed AP-42 attribution forensics when a phase-gate violation is later diagnosed.
- [AP-44 — Expert-blind team averaging (expertise dilution under integrative compromise)](#ap-44--expert-blind-team-averaging-expertise-dilution-under-integrative-compromise): AP-44 is quality degradation in the aggregated answer due to unweighted voting. AP-45 is completeness failure in the aggregated answer due to missing inputs — AP-44 aggregates the wrong set of weights; AP-45 aggregates the wrong (incomplete) set of inputs.

**References.**
- arxiv 2502.14743 "Multi-Agent Coordination Across Diverse Applications: A Survey" (February 2026) — explicitly names absence of lock/queue/barrier primitives as root cause of deadlocks and livelocks across all surveyed frameworks; identifies "phase gates" as a missing coordination primitive; "agents can move but are coupled with each other and unable to progress independently" describes the livelock that occurs when one phase-N agent is blocked waiting for another without a barrier to prevent premature advancement. ([arxiv](https://arxiv.org/abs/2502.14743))
- arxiv 2605.07935 "TraceFix: Repairing Agent Coordination Protocols with TLA+ Counterexamples" (May 2026, Rutgers) — verifies coordination protocols with TLA+ model checker; deadlock/livelock rates reduced from **31.1% to 14.1%** under injected faults using formally verified protocols; the residual 14.1% represents runtime failures (crashed phase-N agents, stale committed tokens, partial writes) that static protocol verification cannot prevent — exactly the gap that runtime barrier primitives and crash-recovery watchdogs fill. ([arxiv](https://arxiv.org/abs/2605.07935))
- arxiv 2605.06320 "Improving the Efficiency of Language Agent Teams with Adaptive Task Graphs" (LATTE, May 2026, Princeton/MIT/Cambridge/NYU) — defines formal `Claim`, `Complete`, and `Release` graph operators with preconditions and postconditions enforcing phase ordering; LATTE's Assign → Claim → Complete → Release sequence is a formal statement that phase N+1 (`Release`) is only valid after phase N (`Complete`) has committed; **code released**; requires an underlying barrier substrate that no Python library provides — confirms the phase-gate gap exists one layer below the orchestration framework. ([arxiv](https://arxiv.org/abs/2605.06320))
- arxiv 2503.13657 "Why Do Multi-Agent LLM Systems Fail?" (MAST study, February 2026) — 1,642 execution traces across 7 open-source frameworks; "most failures originate at system boundaries rather than within individual model calls, with issues related to orchestration, runtime execution, and workflow behavior dominating"; phase-boundary failures (consuming incomplete prior-phase outputs) are a structural system-boundary failure mode not detectable by per-agent unit tests. ([arxiv](https://arxiv.org/abs/2503.13657))
- GitHub practitioner (sigalovskinick, May 2026) — "~20k LOC, 135+ tests" of custom infrastructure for dual-orchestrator agents; explicitly documents **"Flat Task Lists (no phase gates)"** as one of eight recurring failure modes; custom file transaction managers, lock managers, and phase-gate equivalents built to address ordering failures; "each new system requires the same 20k LOC rebuild" confirms the barrier primitive gap is felt in every parallel agent deployment. ([gist](https://gist.github.com/sigalovskinick/6cc1cef061f76b7edd198e0ebc863397))
...
**Symptom.** A work queue shows one or more tasks in "claimed" or "in-progress" state with no heartbeat updates. The orchestrator's wait loop never exits. Pipeline throughput drops to zero. No exception fires, no cost alarm fires, no loop counter increments — the pipeline is simply frozen waiting for a dead agent to call `release()` on a claim it can no longer reach. Compute resources stay allocated, wall-clock time accumulates, and a human eventually intervenes by manually deleting the stale claim record.

**Example.** A five-agent code-generation pipeline assigns sub-tasks via a SQLite-backed work queue. Agent A3 claims task T3, begins generation, then receives a SIGKILL from the container scheduler (OOM, pod eviction, network partition). The claim record — `task_id=T3, agent_id=A3, status=claimed, claimed_at=14:22:07` — remains in the database. Agents A1, A2, A4, and A5 complete their tasks and the orchestrator calls `wait_all_complete()`, which polls the queue and finds one task still "claimed." The pipeline blocks. The container is gone. No timeout fires because `max_iterations` and `max_cost` track API calls, not claim age. After 90 minutes a human checks pod logs and manually deletes the stale record. The pipeline resumes — silently missing T3's output.

**Root cause.**
- Work queue implementations in agent frameworks do not attach TTL-bearing leases to claims — a claim, once made, holds until the claiming agent explicitly calls `release()`.
- No heartbeat / lease-renewal protocol exists between the claiming agent and the queue: there is no mechanism to distinguish "agent is still working" from "agent is dead."
- Orchestrators poll queue status without checking claim age against a freshness threshold — an 8-hour-old claim looks identical to a 2-second-old claim in a standard status check.
- Container and process failures (OOM, pod eviction, network partition, uncaught exception) do not trigger queue-side claim release — no `finally`-block or signal handler can guarantee cleanup across all failure modes in a distributed runtime.

**Mitigations.**
1. **TTL-bearing claim leases.** `WorkQueue.claim(task_id, ttl_secs=60)` stores `expires_at = claimed_at + ttl_secs` alongside the claim record. A background scanner marks expired claims as `AVAILABLE` for reassignment without requiring the dead agent or orchestrator to act — expiry fires automatically.
2. **Claim-health-check heartbeat.** The claiming agent calls `lease.renew()` at `ttl_secs / 3` intervals while the task is in progress. A claim whose `heartbeat_at` timestamp exceeds `ttl_secs` without renewal is treated as orphaned and auto-reassigned, independently of whether the agent process is still reachable from the orchestrator.
3. **Orchestrator watchdog with claim reassignment.** The orchestrator periodically scans for claims where `now() − heartbeat_at > ttl_secs`, increments `attempt_count`, re-queues the task as `AVAILABLE` (up to `max_attempts`), and fires an `on_claim_orphan` callback with `(task_id, agent_id, elapsed_secs)` for alerting and audit.
4. **DEGRADED pipeline state propagation.** When a task cannot be recovered after `max_attempts` orphan cycles, the orchestrator emits a `DEGRADED` signal to dependent phases rather than blocking indefinitely. Downstream agents can proceed with available results under an explicit incompleteness flag or halt with a structured error — either outcome is recoverable; an infinite block is not.

**Detection.**
- **Claim-age histogram.** Any claim whose `now() − claimed_at` exceeds `3× median_claim_duration` with no heartbeat update is an orphan candidate — an alert fires before the orchestrator's wait loop has time to freeze.
- **Work-queue drain stall.** If the queue holds `in-progress` tasks but no completion events arrive within `N seconds`, and heartbeat timestamps are not advancing, a stale-claim deadlock is the likely cause — distinct from normal slow execution, where heartbeats continue even if completions are delayed.
- **Agent-ID liveness cross-reference.** Compare the claiming agent's ID against the runtime's process or pod registry. If the agent process is absent but its claim is live, the claim is confirmed orphaned — fire immediate reassignment rather than waiting for TTL expiry.
- **Attempt-count anomaly.** A task re-queued two or more times without completion indicates a recurring crash at a specific point in the work — a higher-priority diagnostic signal than a one-off retry.

**Related.**
- [AP-27 — Multi-agent concurrent state corruption](#ap-27--multi-agent-concurrent-state-corruption): AP-27 is concurrent within-phase write conflicts on a shared artifact. AP-46 is a temporal blocking failure — the pipeline is serialized behind a dead agent's unreleased claim, with no shared-artifact write occurring.
- [AP-31 — Hallucinated multi-agent consensus](#ap-31--hallucinated-multi-agent-consensus): AP-31 is agents verbally reporting agreement without committed state. AP-46 is the opposite commitment failure: the agent committed to a claim and then vanished, leaving the commitment permanently asserted without a corresponding completion.
- [AP-36 — Agent capacity overload cascade (absent backpressure primitives)](#ap-36--agent-capacity-overload-cascade-absent-backpressure-primitives): AP-36 is throughput collapse when a downstream agent is saturated by excess incoming work. AP-46 is deadlock from the opposite direction — the work queue is not overloaded; one item is permanently stuck in-progress while the entire pipeline waits.
- [AP-42 — Multi-agent failure attribution blackout](#ap-42--multi-agent-failure-attribution-blackout): AP-42 is the forensics problem after a failure has occurred. AP-46 is the live operational problem — the pipeline is frozen now, not retrospectively wrong; the watchdog and claim-reassignment mechanism (AP-46 mitigation 3) is a direct input to AP-42 attribution forensics when a crash is later investigated.
- [AP-45 — Absent phase-gate barrier (parallel agents advancing without inter-phase synchronization)](#ap-45--absent-phase-gate-barrier-parallel-agents-advancing-without-inter-phase-synchronization): AP-45 is a phase ordering failure — phase N+1 advancing prematurely before phase N finishes. AP-46 is phase N never finishing — the dead agent holds the last unclaimed task, preventing the phase from completing at all. AP-45's crash-recovery watchdog (mitigation 3) is downstream of the AP-46 primitive: the watchdog exists precisely to handle the stale-lease scenario that AP-46 documents as its primary failure mode.

**References.**
- GitHub practitioner (sigalovskinick, May 2026) — "~20k LOC, 135+ tests" of custom infrastructure for dual-orchestrator agents; explicitly documents **"Mid-Task Agent Failure"** as one of eight recurring failure modes; custom lock managers and claim-reassignment logic built to address the stale-claim scenario; "each new system requires the same 20k LOC rebuild" confirms the TTL/watchdog primitive gap is felt in every parallel agent deployment. ([gist](https://gist.github.com/sigalovskinick/6cc1cef061f76b7edd198e0ebc863397))
- arxiv 2502.14743 "Multi-Agent Coordination Across Diverse Applications: A Survey" (February 2026) — deadlocks are formally defined as "agents stop moving forever as if being locked"; identifies absence of lock/queue/barrier primitives as root cause across all surveyed frameworks; explicit deadlock scenario: one agent holds a resource indefinitely while all dependent agents wait — the exact stale-claim failure mode. ([arxiv](https://arxiv.org/abs/2502.14743))
- arxiv 2605.07935 "TraceFix: Repairing Agent Coordination Protocols with TLA+ Counterexamples" (May 2026, Rutgers) — reduces deadlock/livelock rates from **31.1% to 14.1%** using formally verified protocols; the residual 14.1% under injected faults represents runtime failures (mid-execution crashes, SIGKILL, network partitions) that formal protocol verification cannot prevent — exactly the failure mode AP-46 documents; runtime TTL/heartbeat/watchdog enforcement is the necessary complement to design-time protocol verification. ([arxiv](https://arxiv.org/abs/2605.07935))
- arxiv 2605.03310 "Coordination as an Architectural Layer for LLM-Based Multi-Agent Systems" (May 2026) — production failure rates 41–87% from coordination defects as causal root; information-controlled empirical isolation confirms the causal link; stale-claim deadlock is a coordination defect that appears as zero-throughput pipeline with no error signal — invisible to standard monitoring until a human intervenes. ([arxiv](https://arxiv.org/abs/2605.03310))
- GitHub `microsoft/autogen#7321` (open 2026) — "Backpressure contract declarations": issue thread documents the stale-claim variant alongside saturation failures — a claiming agent that crashes leaves the task slot permanently locked without a retry mechanism, freezing dependent agents; confirms the claim-reassignment gap exists in a widely-used production framework. ([github.com/microsoft/autogen/issues/7321](https://github.com/microsoft/autogen/issues/7321))
...
**Symptom.** A parallel agent pipeline outputs a result that is syntactically correct and individually reasonable per agent, but globally incoherent — one agent produced a JSON schema summary, another produced a natural-language narrative; one agent optimized for brevity (5 items), another for completeness (200 items); one agent interpreted "current architecture" as the v2 system, another as the legacy v1 system. The merge step either silently concatenates incompatible representations into a nonsense aggregate, or crashes with a type/schema error whose stack trace gives no indication that the root cause is semantic. Retry does not help — the divergence is structural, not transient.

**Example.** A four-agent research pipeline receives the directive: "analyze the codebase and produce a risk assessment." Agent A1 interprets "risk" as security vulnerabilities (outputs CVSS scores). Agent A2 interprets "risk" as performance regressions (outputs latency benchmarks). Agent A3 interprets "risk" as architectural debt (outputs dependency graph analysis). Agent A4 interprets "risk" as business impact (outputs cost projections). Each output is high-quality in isolation. The combiner receives four structurally different documents with no shared schema. It either picks one arbitrarily, concatenates all four into a 40-page document the user cannot act on, or errors: `"cannot merge: inconsistent output schemas."` No log entry identifies semantic divergence at dispatch as the root cause.

**Root cause.**
- Multi-agent frameworks dispatch tasks as natural-language strings with no formal semantic commitment protocol — agents accept text instructions and independently resolve ambiguity at runtime from their local context.
- No standard primitive exists for extracting a shared task contract (objective, scope boundaries, output schema, constraint priority order) from a natural-language directive before agent spawn.
- Parallel agents have no cross-agent semantic visibility during execution — each proceeds in isolation without checking whether its evolving interpretation remains compatible with its peers.
- Combiner agents are designed to aggregate semantically compatible inputs; they have no mechanism to detect or surface semantic-level incompatibility, and no coordination framework provides an upstream semantic alignment gate before the combiner runs.

**Mitigations.**
1. **Pre-dispatch task contract.** Before spawning agents, generate an explicit task contract (structured YAML/JSON) from the natural-language directive: `{objective, scope, output_format, output_schema, constraints, priority_order}`. Each spawned agent receives the contract and acknowledges it before beginning execution. Mismatches between the contract's `output_schema` and the agent's proposed output are surfaced at spawn time, not at merge time.
2. **Mid-execution semantic drift check.** At configurable checkpoints, the orchestrator samples each agent's intermediate output and computes cosine similarity against the task-contract embedding. A similarity drop below a configurable threshold (e.g., 0.70) fires a `SemanticDriftAlert` and optionally injects a correction prompt to the drifting agent before its output is final.
3. **Pre-merge semantic consensus gate.** Before the combiner step, run pairwise intent-compatibility checks across all agent outputs against the shared task contract (the approach from arxiv 2604.16339). Any output pair with low compatibility triggers a re-alignment cycle — agents are asked to reconcile their interpretations — rather than passing incompatible inputs to the combiner.
4. **Structured output schema enforcement on inter-agent handoffs.** Enforce JSON Schema on all inter-agent messages. Schema validation at each handoff surfaces format incompatibility immediately rather than at the final combiner. Note: schema-valid does not imply semantically aligned — schema enforcement catches format drift, not scope drift; it is necessary but not sufficient on its own.

**Detection.**
- **Combiner input divergence metric.** Before the combiner runs, compute pairwise cosine similarity across all agent output embeddings. Mean pairwise similarity below 0.65 indicates likely semantic divergence — flag for human review or trigger a re-alignment cycle before combining.
- **Schema mismatch count at combiner boundary.** Track how often the combiner receives inputs with incompatible JSON schemas or incompatible field sets across runs. Any non-zero rate is a semantic intent divergence signal in the dispatch layer.
- **Post-run semantic audit.** After pipeline completion, embed the original task directive and each agent's output; measure cosine distance between each output and the directive. Outputs with distance > 0.4 from the directive indicate uncorrected scope drift during execution.
- **Retry-invariant combiner failure.** A combiner error that persists across 3+ retries with the same set of agent outputs is structurally caused (semantic divergence) rather than transiently caused (rate limit, network timeout) — route to a semantic resolution path rather than a retry path.

**Related.**
- [AP-27 — Multi-agent concurrent state corruption](#ap-27--multi-agent-concurrent-state-corruption): AP-27 is a physical consistency failure — parallel agents writing incompatible state to a shared artifact in real time. AP-47 is a semantic consistency failure — parallel agents producing semantically incompatible outputs in isolation, with no shared-artifact conflict until the combiner receives them.
- [AP-31 — Hallucinated multi-agent consensus](#ap-31--hallucinated-multi-agent-consensus): AP-31 is agents verbally reporting agreement without committing state — false consensus. AP-47 is genuine independent execution without agreement: agents never claim to agree; they diverge silently because no mechanism exists to align their semantic interpretations before execution begins.
- [AP-42 — Multi-agent failure attribution blackout](#ap-42--multi-agent-failure-attribution-blackout): AP-42 is the forensics problem — after a failure, which agent/step was responsible? AP-47 is a prevention problem: the task contract and semantic drift check stop divergence before the combiner rather than attributing the failure afterward. Pre-dispatch task contracts (AP-47 mitigation 1) are direct inputs to AP-42 attribution — a contract deviation is a traceable causal root.
- [AP-44 — Expert-blind team averaging (expertise dilution under integrative compromise)](#ap-44--expert-blind-team-averaging-expertise-dilution-under-integrative-compromise): AP-44 is quality degradation when agents share the same objective but an averaging mechanism dilutes the best answer. AP-47 is a coherence failure when agents share the same directive but independently resolve its meaning differently — the outputs cannot be averaged because they are incommensurable, not merely low-quality.
- [AP-45 — Absent phase-gate barrier (parallel agents advancing without inter-phase synchronization)](#ap-45--absent-phase-gate-barrier-parallel-agents-advancing-without-inter-phase-synchronization): AP-45 is a temporal ordering failure — phase N+1 consumes incomplete phase-N outputs. AP-47 is a semantic alignment failure — phase N produces complete but semantically incompatible outputs. Both manifest at the phase-N → phase-N+1 handoff boundary but via orthogonal mechanisms; a pipeline can exhibit both simultaneously.

**References.**
- arxiv 2604.16339 "Semantic Consensus: Process-Aware Conflict Detection and Resolution for Enterprise Multi-Agent LLM Systems" (April 2026) — **79% of multi-agent failures are from specification and coordination issues, not model capability**; explicitly names "Semantic Intent Divergence" as a distinct failure mode distinct from physical coordination failures; the Semantic Consensus Framework achieves **100% workflow completion** across AutoGen, CrewAI, and LangGraph where baseline coordination mechanisms fail; confirms that a pre-merge semantic alignment primitive fills the gap no current framework provides. ([arxiv](https://arxiv.org/abs/2604.16339))
- arxiv 2601.04170 "Agent Drift: Quantifying Behavioral Degradation in Multi-Agent LLM Systems Over Extended Interactions" — **inter-agent misalignment = 36.9% of all observed failure modes**; production failure rates 41–86.7% across major frameworks; token duplication 53–86% indicating agents producing redundant rather than complementary outputs — a symptom of absent semantic task partitioning at dispatch time. ([arxiv](https://arxiv.org/abs/2601.04170))
...
**Root cause.** Tool identity is encoded in a low-dimensional linear subspace of model residual-stream activations: the activation gap between the top-1 and top-2 tool identity directions is a direct, mechanistically interpretable proxy for selection certainty (2605.07990). Separately, models encode reliable tool-necessity signals in their hidden states even when they fail to act on those signals during generation — generating a tool call despite an internal representation that the tool is not needed (arxiv 2605.09252 When2Tool). Both signals exist in the model at generation time and are discarded before any runtime guard runs: the agent runtime receives only the generated token string (tool name + arguments), not the activation state that produced it.

**Why it's a gap.** The mechanistic evidence (2605.07990, 12 instruction-tuned models, 270M–27B) shows that tool identity is steerable via activation vectors at 77–100% accuracy (93–100% at 4B+), that the activation gap predicts errors 14–21x more accurately than random on Gemma 3 12B/27B, and that the JSON arguments autoregressively conform to whichever tool's schema the activation is steered toward — confirming the signal is both real and actionable. The behavioral evidence (2605.09252 When2Tool, 18 environments, 15 single-hop + 3 multi-hop) confirms models already have tool-necessity signals and that training-free prompting (Prompt-only, Reason-then-Act baselines) can partially surface them. Two independent papers from the same week converge: models know when they are uncertain and whether a tool is needed, but no agent runtime exposes either signal to a pre-execution gate.

**Mitigations.**
1. **Activation-gap pre-execution gate:** at generation time, measure the logit gap between the top-1 and top-2 tool-name token candidates (accessible without full activation hooks on most inference stacks); below a tuned threshold, defer execution and inject a disambiguation step ("is `tool_A` or `tool_B` better suited given the query?") or fall back to CoT reasoning.
2. **Reason-then-Act invocation wrapper:** insert a structured reasoning step before tool-name generation — "given the current query, which tool is necessary and why?" — forcing the model to surface its internal necessity signal in text before committing to a tool name; the When2Tool Reason-then-Act baseline demonstrates partial gap closure without fine-tuning.
3. **Competing-tool disambiguation probe:** when the logit gap is below threshold, sample a second candidate tool name at higher temperature and run a lightweight arbitration prompt ("tool A vs. tool B: which is correct for this task?") before executing either; reject execution if arbitration is inconclusive and escalate to the user or a higher-capability model.
4. **Selection-confidence telemetry:** log top-1 vs. top-2 tool-name logit gap per call; alert when the rolling 5th-percentile confidence drops below the calibration threshold — systematic low-confidence in a tool class signals a schema description problem (AP-33) or a format compilation issue (AP-25) that a confidence monitor surfaces before wrong calls accumulate.

**Detection signals.**
1. **Wrong-tool rate by confidence decile:** stratify tool calls by top-1/top-2 logit gap decile post-hoc; a bottom-decile wrong-call rate >10× the top-decile rate confirms the activation-gap signal is active and that a pre-execution gate would intercept the failure class.
2. **Retry-invariant wrong-tool pattern:** same wrong tool selected on ≥2 retries without error = systematic low-confidence selection, not a transient failure; standard retry logic amplifies the problem rather than recovering from it.
3. **High-entropy tool-name prefix tokens:** logit entropy over the first 1–3 characters of the tool name is a lightweight proxy for selection uncertainty accessible without activation-level hooks on any inference stack; a session-level entropy spike precedes wrong-tool surges.
4. **When2Tool-class accuracy probe:** run a set of tool-necessary vs. tool-unnecessary task probes against the deployed model; tasks where the model fails to align its generated tool call with its hidden-state necessity signal identify AP-48 exposure points in the current deployment configuration.

**Distinct from:**
- [AP-09 — Tool-selection lock-in](#ap-09--tool-selection-lock-in): AP-09 is an agent that *always* reaches for the same tool regardless of the task — high-confidence wrong selection from a habitual bias. AP-48 is *low*-confidence wrong selection when multiple tools are plausible — the model is uncertain, not habitual; the activation-gap signal is large for AP-09 (the wrong tool wins decisively), small for AP-48.
- [AP-25 — Tool-schema wire-format incompatibility](#ap-25--tool-schema-wire-format-incompatibility): AP-25 is a parsing failure — the model cannot decode the schema format and produces a null or malformed call even when it would have selected the right tool. AP-48 is a selection-confidence failure — schema format and parsing are correct, but the model is uncertain which of several correctly-parsed tools to invoke.
- [AP-29 — Unconditional tool invocation (tool-use tax)](#ap-29--unconditional-tool-invocation-tool-use-tax): AP-29 is the decision between *calling any tool* vs. *reasoning directly* — the model invokes tools when CoT would produce a better or cheaper result. AP-48 is the decision between *which specific tool to call* once a tool call has already been committed — an orthogonal failure within the already-committed-to-tool-use branch.
- [AP-33 — Non-functional tool description bias](#ap-33--non-functional-tool-description-bias): AP-33 is selection corruption driven by *external* description surface features (assertive cues, maintenance claims) that inflate one tool's apparent priority. AP-48 is *internal* model uncertainty when multiple tools are genuinely competing — the activation gap is small because the task semantics do not clearly discriminate between options, not because description text has been manipulated.

**Empirical anchors.**
- arxiv 2605.07990 "Tool Calling is Linearly Readable and Steerable in Language Models" (May 2026) — mechanistic interpretability study across 12 instruction-tuned models (Gemma 3, Qwen 3, Qwen 2.5, Llama 3.1, 270M–27B): tool identity is linearly readable and steerable via residual-stream activation vectors; adding the mean-difference between two tools' directions switches selection at **77–100% accuracy** (93–100% at 4B+); the activation gap between top-1 and top-2 tool choices predicts errors **14–21x more accurately** on Gemma 3 12B and 27B; low-gap queries produce that many more wrong calls than high-gap queries; arguments autoregressively conform to the steered tool's schema. No OSS package. ([arxiv](https://arxiv.org/abs/2605.07990))
- arxiv 2605.09252 "LLM Agents Already Know When to Call Tools — Even Without Reasoning" (May 2026) — introduces When2Tool, a benchmark of 18 environments (15 single-hop, 3 multi-hop) spanning three tool-decision categories (computational scale, knowledge boundaries, execution reliability); models encode reliable tool-necessity signals in hidden states even when they fail to act on them during generation; Prompt-only and Reason-then-Act training-free baselines partially bridge the gap; provides the behavioral benchmark complement to the mechanistic evidence in 2605.07990 — both confirm that models already possess internal signals a pre-execution gate can surface. No OSS package. ([arxiv](https://arxiv.org/abs/2605.09252))
- arxiv 2605.00737 "To Call or Not to Call: A Framework to Assess and Optimize LLM Tool Calling" (May 2026) — formal decision-theoretic framework (necessity × utility × affordability) for tool invocation decisions; "self-decisions made by models today to call tools are far from optimal in terms of accuracy"; "optimal tool calling can achieve much better performance than always calling tools with significantly fewer calls"; provides the formal underpinning that the model's generation-time signal is neither optimal nor unavoidable — a gate can improve it. ([arxiv](https://arxiv.org/abs/2605.00737))
...
**Root cause.** Memory stores (vector databases, JSONL files, key-value stores) commit facts with content embeddings and optional metadata but apply no time-to-live (TTL) or decay function to stored entries. Retrieval ranking is cosine-similarity over the content embedding with no temporal discounting: a fact from 6 hours ago ranks equally with a fact from 30 seconds ago if embeddings are similar. As facts change in the external environment (API schemas, user preferences, world state, task constraints), the stored representations fall out of sync while retrieval confidence remains unchanged. No standard memory backend provides a configurable decay function, staleness score, or validity-window constraint; no write-path middleware enforces expiry dates or confidence-decay curves on committed entries.

**Why it's a gap.** arxiv 2605.03675 MemTier quantifies this as a time-dependent degradation problem with a concrete production cost: "incoherence accumulates over hours as facts go stale without TTL/decay enforcement — point-in-time contradiction detection alone is insufficient." arxiv 2605.06527 STALE confirms the best model achieves only 55.2% on implicit-conflict scenarios — situations where a stored fact and a current observation contradict without an explicit flag. arxiv 2603.11768 SSGM formalizes "intrinsic drift" (knowledge conflict as facts evolve) and proposes "forgetting-by-design," naming no production-grade OSS middleware that implements it. The MemTier degradation is not recoverable by retrieval-layer improvements alone: a retrieval reranker that sees equally-confident stale and fresh facts cannot correct for absent TTL/decay metadata that was never committed at write time. The write-path gap is the structural cause; a decay-aware write middleware is the minimal intervention.

**Mitigations.**
1. **TTL-tagged write path:** commit every memory entry with `expires_at` = `write_time` + domain-specific TTL (e.g., 1h for real-time API state, 24h for session context, 7d for long-term preferences); retrieval layer filters expired entries before embedding search runs.
2. **Confidence-decay weighting:** alongside the content embedding, store an initial `confidence` score and a `decay_rate`; retrieval ranking applies `current_confidence = initial_confidence × exp(–decay_rate × elapsed_seconds)` as a multiplicative weight alongside cosine similarity; stale facts surface only when no fresher alternative exists.
3. **Staleness-aware re-validation:** before committing a retrieved fact to an active tool call, run a lightweight LLM validation step ("is this fact still likely valid given current task context and elapsed time?"); if validation returns uncertain, re-query the external source or escalate to the user before acting.
4. **MEMAUDIT periodic freshness audit:** run MEMAUDIT-style audits (arxiv 2605.02199) on memory store snapshots to measure what fraction of stored entries are expired, contradicted, or below a freshness threshold; fire alerts when stale-entry fraction exceeds 15% to trigger proactive cleanup before degradation reaches the 14-point threshold.

**Detection signals.**
1. **Tool-success rate trend over session age:** plot tool execution success vs. session age in hours; a downward slope steeper than –0.2 pp/hour over 6+ hours indicates TTL-induced coherence degradation (MemTier baseline: –14 pp over 72 h ≈ –0.19 pp/hour).
2. **Retrieved-memory age distribution:** at retrieval time, log the `write_time` of each returned entry; median retrieved-entry age >2× the domain's expected TTL is a leading indicator of stale-fact reliance.
3. **Contradiction rate with recent observations:** compare retrieved memory claims against the current turn's direct observations using a lightweight entailment probe; contradiction rate >10% signals stale write-path entries dominating retrieval.
4. **STALE-class probe on deployed agent:** periodically replay STALE implicit-conflict benchmark scenarios (arxiv 2605.06527) against the deployed agent; accuracy below 70% on implicit-staleness probes indicates the retrieval path is not filtering superseded entries.

**Distinct from:**
- [AP-24 — Memory write-path accumulation (unbounded commit without salience gating)](#ap-24--memory-write-path-accumulation-unbounded-commit-without-salience-gating): AP-24 is a *quantity* problem — agents commit every observation, accumulating irrelevant noise from the start. AP-49 is a *temporal* problem — agents commit facts correctly at write time, but those facts decay without TTL enforcement. Mitigations are orthogonal: salience gating reduces commit volume; TTL/decay enforcement corrects for temporal invalidity regardless of commit volume.
- [AP-37 — Overconfident single-belief memory commit (under partial observability)](#ap-37--overconfident-single-belief-memory-commit-under-partial-observability): AP-37 is a *probabilistic* problem at write time — the agent collapses ambiguous observations into overconfident single beliefs before TTL metadata would even be relevant. AP-49 assumes the initial belief was correct and targets the temporal window within which it remains valid.
- [AP-39 — Memory control flow hijacking (adversarial retrieval steering)](#ap-39--memory-control-flow-hijacking-adversarial-retrieval-steering): AP-39 is an *adversarial* problem — maliciously crafted entries injected to dominate retrieval. AP-49 is a *structural* problem — legitimately committed, non-adversarial entries becoming stale. Different threat models, different interventions.
- [AP-40 — Cooperative intent erosion from unbounded memory accumulation (Memory Curse)](#ap-40--cooperative-intent-erosion-from-unbounded-memory-accumulation-memory-curse): AP-40 is a *behavioral* consequence of accumulated conflict history in multi-agent cooperation settings. AP-49 is a *single-agent* correctness failure from temporal drift — stale tool-call grounding, not behavioral erosion from conflict history.
- [AP-32 — Flat multi-agent memory (absent memory scope isolation)](#ap-32--flat-multi-agent-memory-absent-memory-scope-isolation): AP-32 is a *namespace* problem — all agents sharing an unsegmented store with no provenance isolation. AP-49 is a *temporal freshness* problem — correctly scoped entries becoming invalid over time. Both are write-path gaps but at orthogonal dimensions: namespace vs. time.

**Empirical anchors.**
- arxiv 2605.03675 "MemTier: Tiered Memory Architecture and Retrieval Bottleneck Analysis" (May 2026) — long-running agents lose **14 percentage points** of tool execution success over 72-hour windows due to memory coherence degradation; root cause is incoherence accumulation as facts go stale without TTL/decay enforcement; proposes episodic JSONL store, weighted retrieval with five-signal attention, and async consolidation daemon; confirms write-path TTL/decay enforcement is a **time-dependent degradation problem**: incoherence is not detectable at point-in-time retrieval and requires freshness metadata committed at write time. No OSS package. ([arxiv](https://arxiv.org/abs/2605.03675))
- arxiv 2605.06527 "STALE: Can LLM Agents Know When Their Memories Are No Longer Valid?" (May 2026) — benchmark testing whether agents can detect when previously valid memories have been superseded; best model achieves only **55.2%** on implicit-conflict scenarios (where the stored memory and a current observation contradict without an explicit staleness flag); confirms write-path staleness is an unsolved production problem that retrieval reranking cannot fix: if no freshness signal was committed at write time, the retrieval layer cannot distinguish a stale fact from a current one. ([arxiv](https://arxiv.org/abs/2605.06527))
- arxiv 2603.11768 "Governing Evolving Memory in LLM Agents: SSGM Framework" (March 2026) — formalizes "intrinsic drift" (knowledge conflict between facts as the world changes) and proposes "forgetting-by-design" as an architectural requirement; notes that no production-grade middleware implements scheduled decay, validity windows, or conflict-triggered expiry; confirms the write-path gap extends beyond point-in-time contradiction detection to temporal governance — a composable TTL/decay middleware operationalizes the SSGM framework without requiring full stack adoption. ([arxiv](https://arxiv.org/abs/2603.11768))
- arxiv 2605.02199 "MEMAUDIT: An Exact Package-Oracle Evaluation Protocol for Budgeted Long-Term LLM Memory Writing" (May 2026) — turns write-path memory selection into a finite, auditable optimization problem; benchmarks Mem0, A-Mem, and Letta stores; confirms existing systems cannot self-audit what they preserve under storage budgets or over time; a periodic MEMAUDIT-style freshness audit is the detection primitive that surfaces AP-49 exposure before the 14-point degradation accumulates. ([arxiv](https://arxiv.org/abs/2605.02199))

---

## AP-50 — Context-provenance blindness (absent runtime provenance graph for differential trust)

**TL;DR.** Agent runtimes ingest context from multiple sources — system prompt, user message, tool returns, retrieved documents, memory, sub-agent outputs — into a single undifferentiated context window with no per-fragment source provenance. Without a provenance graph, the model applies uniform attention trust to all content: an EXTERNAL-tier web-retrieved fragment poisoned with override instructions receives the same implicit weight as a SYSTEM-tier directive. Channel-isolated defenses (AP-01 external-fetch sanitization; AP-41 tool-return sanitization) each guard one ingestion path but cannot detect adversarial payloads deliberately split across two or more trusted channels.
...
**Mitigations.**
1. **Runtime provenance graph:** at every context ingestion point tag each fragment with `{source_type, source_id, trust_tier, injection_path, ingested_at}`; use a four-tier trust hierarchy — SYSTEM > AGENT_INTERNAL > USER > TOOL_RETURN > RETRIEVED > EXTERNAL; propagate inherited trust tier through all derived fragments (summaries, chunks, sub-agent outputs inherit from their origin source).
2. **Trust-tier-aware context assembly:** preserve provenance tags as structural annotations in the assembled window; instruct the model via system prompt to weight claims proportionally to trust tier when resolving contradictions or filling underspecified parameters; log which provenance tiers contributed to each tool-call argument value.
3. **Cross-channel injection pattern detection (ARGUS-class):** at ingestion time apply a lightweight instruction-density probe to each fragment; flag any EXTERNAL/RETRIEVED fragment with instruction-verb density >0.15 per sentence AND cross-channel correlation — same instruction pattern detected in ≥2 source types within one agent turn — as a multi-source injection attempt before the fragment enters the context window.
4. **Provenance audit log at tool-call decision points:** emit a provenance graph snapshot (fragment list, trust tiers, source IDs contributing to the current window) immediately before each tool call; retain for forensic replay when post-execution output is anomalous.

**Detection signals.**
1. **Trust-tier boundary proximity:** EXTERNAL-tier or RETRIEVED-tier fragment with instruction-density >0.15 assembled within N tokens of a SYSTEM-tier fragment in the context window — proximity amplifies influence under standard attention, making boundary violations a leading indicator.
2. **Cross-channel same-instruction correlation:** identical or near-identical override instruction detected in ≥2 source types (e.g., web-search result and tool return) within one agent turn — hallmark of a deliberately fragmented multi-channel payload.
3. **Provenance gap in tool-call argument:** argument value draws from a fragment absent from the provenance graph — indicates an injection point that bypassed provenance instrumentation entirely.
4. **ARGUS-class probe at deploy time:** inject a known multi-source payload (fragments individually benign per channel, collectively forming an override directive) at test time; if agent behavior changes versus the clean baseline, provenance gate is absent.

**Distinct from:**
- [AP-01 — Prompt injection via tool output](#ap-01--prompt-injection-via-tool-output): AP-01 is a single-channel attack — the entire malicious payload arrives in one external-fetch result. AP-50 is the multi-channel structural gap: each individual fragment is locally benign; the override emerges only in cross-channel combination. AP-01 mitigations are necessary but target one ingestion path; AP-50 provenance graph is the cross-channel coordination layer that makes channel-isolated defenses jointly sufficient.
- [AP-41 — Tool-return content injection (trusted-channel indirect injection)](#ap-41--tool-return-content-injection-trusted-channel-indirect-injection): AP-41 is injection delivered entirely via a registered tool's return value — a single elevated-trust channel. AP-50 targets the structural absence of provenance tracking across all channels, enabling multi-source fragmented payloads that AP-41 sanitization alone cannot catch because neither fragment alone triggers a per-channel threshold.
- [AP-17 — RAG retrieval poisoning](#ap-17--rag-retrieval-poisoning): AP-17 is a write-time attack — attacker corrupts the retrieval index before the agent queries it. AP-50 is a runtime ingestion attack — fragments from multiple sources are legitimately retrieved but assembled without provenance boundaries. AP-17's index-integrity defenses address corpus corruption; AP-50's provenance graph addresses runtime assembly trust regardless of corpus integrity.
- [AP-35 — Long-horizon tool-attack chain (sequential stealth exploitation)](#ap-35--long-horizon-tool-attack-chain-sequential-stealth-exploitation): AP-35 is a multi-turn temporal attack — adversarial payload distributed across N separate tool calls over time. AP-50 is a single-turn spatial attack — adversarial payload fragmented across N source types within one context window assembly. AP-35 defends with shadow-memory trajectory tracking across turns; AP-50 defends with provenance graph within a single assembly.
- [AP-43 — Coarse-grained tool authorization (binary allow/deny without per-call scope enforcement)](#ap-43--coarse-grained-tool-authorization-binary-allowdeny-without-per-call-scope-enforcement): AP-43 is the authorization decision at tool-call execution time — which calls are permitted given caller identity, scope, and privilege. AP-50 is the context ingestion provenance gap — what trust level was assigned to the fragments that shaped the tool-call decision. AP-43 DEFER/STEP_UP verdicts catch suspicious calls regardless of context origin; AP-50 provenance graph catches context manipulation before the authorization layer is reached.

**Empirical anchors.**
- arxiv 2605.03378 "ARGUS: Context-Aware Prompt Injection Defense with Provenance Graph" (May 2026) — builds a runtime context provenance graph that tags each fragment with source identity and injection path at ingestion; provenance-aware detection substantially outperforms channel-isolated baselines on multi-source injection scenarios; confirms that absent provenance infrastructure is the unifying root cause across AP-01, AP-41, and multi-channel combination attacks — each channel-isolated defense is independently insufficient when an adversary can write to any two low-trust channels and fragment a payload to evade per-channel thresholds. ([arxiv](https://arxiv.org/abs/2605.03378))
